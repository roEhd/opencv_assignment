...
int main() {
	...
	output = gaussianfilter_separate(input_gray, 1, 1, 1, "adjustkernel");
...
Mat gaussianfilter_separate(const Mat input, int n, float sigmaT, float sigmaS, const char* opt) {
	...
	kernel_temp = Mat::zeros(kernel_size, 1, CV_32F);
	Mat output = Mat::zeros(row, col, input.type());
			...
			if (!strcmp(opt, "zero-paddle")) {
				...
				for (int a = -n; a <= n; a++) {
					...
					for (int b = -n; b <= n; b++) {
						...
						if ((i + a <= row - 1) && (i + a >= 0) && (j + b <= col - 1) && (j + b >= 0)) { //if the pixel is not a border pixel
							kernel_temp.at<float>(a + n, 0) += kernel_t.at<float>(0, b + n) * (float)(input.at<G>(i + a, j + b));
						}

					}

					sum1 += kernel_s.at<float>(a + n, 0) * kernel_temp.at<float>(a + n, 0);
					kernel_temp.at<float>(a + n, 0) = 0.0;
				}
				...
				for (int a = -n; a <= n; a++) {
					for (int b = -n; b <= n; b++) {
						...
						kernel_temp.at<float>(a + n, 0) += kernel_t.at<float>(0, b + n) * (float)(input.at<G>(tempa, tempb));

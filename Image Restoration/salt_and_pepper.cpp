...
int main() {
	...
	Mat noise_Gray = Add_salt_pepper_Noise(input_gray, 0.005f, 0.005f);
	Mat noise_RGB = Add_salt_pepper_Noise(input, 0.005f, 0.005f);

	// Denoise, using median filter
	int window_radius = 2;
	Mat Denoised_Gray = Salt_pepper_noise_removal_Gray(noise_Gray, window_radius, "adjustkernel"); //zero-padding, mirroring, adjustkernel
...
Mat Salt_pepper_noise_removal_Gray(const Mat input, int n, const char* opt)
{
	...
	Mat kernel = Mat::zeros(kernel_size * kernel_size, 1, input.type());	// initialize median filter kernel
			...
			if (!strcmp(opt, "zero-padding")) {
				...
				for (int x = -n; x <= n; x++) {
					for (int y = -n; y <= n; y++) {
						if ((i + x <= row - 1) && (i + x >= 0) && (j + y <= col - 1) && (j + y >= 0)) {
							...
							kernel.at<G>(kernel_size * (x + n) + (y + n), 0) = 0;
						}					
					}
				}
			}
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						kernel.at<G>(kernel_size * (x + n) + (y + n), 0) = input.at<G>(tempa, tempb);
					}
				}
				median = n;
			}
			...
			if (!strcmp(opt, "zero-padding")) {
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						if ((i + x <= row - 1) && (i + x >= 0) && (j + y <= col - 1) && (j + y >= 0)) {
							...
							kernel.at<G>(kernel_size * (x + n) + (y + n), 0) = 0;
							kernel.at<G>(kernel_size * (x + n) + (y + n), 1) = 0;
							kernel.at<G>(kernel_size * (x + n) + (y + n), 2) = 0;
						}
					}
				}
				median = n;
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						kernel.at<G>(kernel_size * (x + n) + (y + n), 0) = input.at<C>(tempa, tempb)[0];
						kernel.at<G>(kernel_size * (x + n) + (y + n), 1) = input.at<C>(tempa, tempb)[1];
						kernel.at<G>(kernel_size * (x + n) + (y + n), 2) = input.at<C>(tempa, tempb)[2];
					}
				}
				median = n;
			}

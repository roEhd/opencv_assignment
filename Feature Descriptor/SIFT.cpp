...
void findPairs(vector<KeyPoint>& keypoints1, Mat& descriptors1,
	...
	for (int i = 0; i < descriptors1.rows; i++) {
		...
		if (ratio_threshold && nn != -1) {
			...
			double minDist = 1e6;
			double temp;
			Mat v = descriptors2.row(0);
			double min = euclidDistance(desc1, v);
			v = descriptors2.row(1);
			double second = euclidDistance(desc1, v);
			...
			for (int i = 2; i < descriptors2.rows; i++) {
				Mat v = descriptors2.row(i);		
				double dist = euclidDistance(desc1, v);
				if (min > dist) {	
					temp = min;
					min = dist;
					second = temp;

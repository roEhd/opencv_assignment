			...
			if (corner_mat.at<uchar>(i-radius, j-radius) == 1) {
				...
				for (int k = -radius; k <= radius; k++) {
					for (int l = -radius; l <= radius; l++) {
						...
						if (input_mirror.at<float>(i + k, j + l) > max && i + k >= 0 && i + k < row && j + l >= 0 && j + l < col) {

							corner_mat.at<uchar>(x-radius, y-radius) = 0;
							max = input_mirror.at<float>(i + k, j + l);
							x = i + k;
							y = j + l;							
							corner_mat.at<uchar>(x-radius, y-radius) = 1;
						}
					}
				}			
			}

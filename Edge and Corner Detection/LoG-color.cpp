...
Mat Gaussianfilter(const Mat input, int n, double sigma_t, double sigma_s) {
	...
	for (int i = n; i < row + n; i++) {
		for (int j = n; j < col + n; j++) {
			...
			for (int a = -n; a <= n; a++) {
				for (int b = -n; b <= n; b++) {
					sum_r += kernel.at<double>(a + n, b + n) * ((double)input_mirror.at<Vec3d>(i + a, j + b)[0]);
					sum_g += kernel.at<double>(a + n, b + n) * ((double)input_mirror.at<Vec3d>(i + a, j + b)[1]);
					sum_b += kernel.at<double>(a + n, b + n) * ((double)input_mirror.at<Vec3d>(i + a, j + b)[2]);
				}
			}
...
Mat Laplacianfilter(const Mat input) {
	...
	for (int i = n; i < row + n; i++) {
		for (int j = n; j < col + n; j++) {
			...
			output.at<double>(i - n, j - n) = (double)(sum_r + sum_g + sum_b) / 3;
